...
function getWorkspaceChanges(workspaces, changedFiles) {
    ...
    function visit(base) {
        ...
        if (known != null)
            return known;
        const changed = [...workspace.dependencies].some(visit) ||
            files.some(file => file.startsWith(`${base}${path_1.default.sep}`));
...
function fetchComparisonBase() {
    ...
    if (!hasCommit(base)) {
        ...
        if (base.startsWith('HEAD')) {
            (0, child_process_1.spawnSync)('git', ['fetch', 'origin', '--deepen=1'], {
                stdio: 'inherit',
                timeout: 10000
            });
        }
        else {
            (0, child_process_1.spawnSync)('git', ['fetch', 'origin', base], {
                stdio: 'inherit',
                timeout: 10000
            });
            (0, child_process_1.spawnSync)('git', ['branch', base, 'FETCH_HEAD'], {
                stdio: 'inherit',
                timeout: 2000
            });
        }
...
function parseRevision(commit) {
    const { stdout } = (0, child_process_1.spawnSync)('git', ['rev-parse', '--verify', commit], {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.glob = void 0;
const globby_1 = __nccwpck_require__(3271);
...
const workspace_1 = __nccwpck_require__(8132);
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        const now = new Date();
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.visit = exports.discover = exports.detect = void 0;
...
function detect() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const matches = yield (0, glob_1.glob)(packages);
        for (const base of matches) {
            if ((yield promises_1.default.stat(base)).isDirectory()) {
                core.debug(`cargo: visit ${base}`);
                const workspace = yield visit(base);
                if (workspace != null)
...
function discover() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const contents = yield promises_1.default.readFile('Cargo.toml', 'utf-8');
...
function visit(base) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return workspace;
        function visitDependencies(deps) {
            if (!deps)
                return;
            ...
            for (const [name, dep] of Object.entries(deps)) {
                ...
                if (typeof dep === 'object' && dep.path) {
                    workspace.dependencies.add(path_1.default.resolve(base, dep.path));
                }
            }
        }
    });
...
const path_1 = __importDefault(__nccwpck_require__(1017));
const cargo = __importStar(__nccwpck_require__(3738));
const npm = __importStar(__nccwpck_require__(3554));
const pnpm = __importStar(__nccwpck_require__(6581));
function detect() {
    return __awaiter(this, void 0, void 0, function* () {
        const detected = yield Promise.all([
            cargo.detect(),
            npm.detect(),
            pnpm.detect()
        ]);
...
function merge(detected) {
    ...
    for (const results of detected) {
        ...
        for (const [fullPath, workspace] of results) {
            ...
            if (!merged.has(base)) {
                merged.set(base, Object.assign(Object.assign({}, workspace), { dependencies: new Set([...workspace.dependencies].map(toRelative)) }));
            }
            else {
                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                const existing = merged.get(base);
                merged.set(base, Object.assign(Object.assign(Object.assign({}, existing), workspace), { aliases: new Set([...existing.aliases, ...workspace.aliases]), dependencies: new Set([
...
function toRelative(fullPath) {
    return path_1.default.relative(process.env.GITHUB_WORKSPACE || process.cwd(), fullPath);
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.resolve = exports.visit = exports.discover = exports.detect = void 0;
const promises_1 = __importDefault(__nccwpck_require__(3292));
const path_1 = __importDefault(__nccwpck_require__(1017));
const core = __importStar(__nccwpck_require__(2186));
const glob_1 = __nccwpck_require__(7975);
...
function discover() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const contents = yield promises_1.default.readFile('package.json', 'utf-8');
...
function resolve(workspaces) {
    const packages = new Map([...workspaces.values()].map(workspace => [workspace.npm.name, workspace.path]));
    for (const workspace of workspaces.values()) {
        for (const dependency of workspace.npm.dependencies) {
            const peer = packages.get(dependency);
            if (peer)
                workspace.dependencies.add(peer);
        }
    }
}
...
const glob_1 = __nccwpck_require__(7975);
const npm_1 = __nccwpck_require__(3554);
function detect() {
    return __awaiter(this, void 0, void 0, function* () {
        const decl = yield discover();
        if (decl == null)
            return null;
        const workspaces = new Map();
        const matches = yield (0, glob_1.glob)(decl.packages);
        for (const base of matches) {
            if ((yield promises_1.default.stat(base)).isDirectory()) {
                core.debug(`pnpm: visit ${base}`);
                const workspace = yield (0, npm_1.visit)(base);
                if (workspace != null)
                    workspaces.set(base, workspace);
            }
        }
        (0, npm_1.resolve)(workspaces);
        return workspaces;
    });
}
exports.detect = detect;
function discover() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const contents = yield promises_1.default.readFile('pnpm-workspace.yaml', 'utf-8');
